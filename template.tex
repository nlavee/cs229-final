...
From this feature map, we basically have a vector containing 1 or 0, indicating whether the n-gram at the index is present in $S_1$. The same would be done for another $S_2$. If we take the dot product between these two feature maps, the only time where a value of dot product is 1 would be if both $S_1$ and $S_2$ has the n-grams at the current index, making the value in the fearure map vector to be 1 for each. If the n-grams is missing on both string, or present on only 1, then we'll always have 0. This aligns with the current kernel method that only increment by 1 if an n-gram is present on both string.
...
On a related note, back in PSET2 Q3, we have discussed constructing Kernels. The feature mapping for $V'$ can be viewed as $K(x,z) = K_3(\phi(x),\phi(x))$ where the initial mapping to restricted $V'$ is the $\phi(x)$. Since we know $K_{V,n}(S_1, S_2)$ is a valid kernel, so by extension, $K_{V',n}(S_1, S_2)$ is also a valid kernel.
 ...
 $S_2$= "an apple an orange" will give us $K_{V, 2}(S_1, S_2) = K_{V',2}(S_1, S_2) = 1$ since both (an apple, an orange) are mapped to a singular (UNK UNK) and will only be counted once for $V'$. For $V$, (an apple) is counted.. \\
 $S_2$= "an apple that saw" will give us $K_{V,2n}(S_1, S_2) = K_{V', 2}(S_1, S_2) = 2$ since (an apple, apple that, that saw) are mapped to (UNK UNK, UNK saw), which are both counted in $V'$. For $V$, (an apple, that saw) is also counted.\\
However, \\
$S_2$= "an apple that that" will give us $K_{V, 2}(S_1, S_2) =2 > K_{V', 2}(S_1, S_2) = 1$. For $V'$, (an apple, apple that, that that) is mapped to (UNK UNK), and is counted once as common string using $V'$. For $V$, we count both "an apple" and "that that", giving us 2.\\
